...
const Team = ({ team, expandRoster, expandMatchup, matchup, showRoster, currentMatchup, mobile, weeks }) => {
	...
	return (
		<Table.Row className={(showRoster && showRoster !== team.id) || (matchup && matchup !== team.id) ? `hide` : ''}>
			<Table.Cell>
				<Header as='h4' image>
					<Image src={team.logo} rounded size='mini' />
					<Header.Content>
						{team.tag}
						<Header.Subheader className='team'>{team.teamName}</Header.Subheader>
			...
			<Table.Cell>
				<Header as='h4' image>
					{!mobile && opponent.logo ? <Image src={opponent.logo} rounded size='mini' /> : ''}
					<Header.Content>
